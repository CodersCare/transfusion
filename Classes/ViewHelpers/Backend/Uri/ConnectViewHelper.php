 ...
 */
final class ConnectViewHelper extends AbstractViewHelper
{
    ...
    public static function renderStatic(
        ...
        foreach ($arguments['tables'] as $table) {
            if (empty($GLOBALS['TCA'][$table]['ctrl']['transOrigPointerField'])) {
                throw new InvalidArgumentException(
                        'Table must be translatable and provide a transOrigPointerField to be connected. This table can\'t be connected',
                        1706372241
                );
            }
        }
        ...
        return (string)$uriBuilder->buildUriFromRoute('language_connect', $params);
    }

    public function initializeArguments(): void
    {
        $this->registerArgument('page', 'int', 'uid of record to be edited, 0 for creation', true);
        $this->registerArgument('language', 'int', 'id of the language to be connected', true);
